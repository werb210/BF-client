...
// Single source of truth for Step 1 → Step 2 category eligibility,
// hard-stop popups, multi-leg construction, and closing-costs companion
// routing. Imported by Step1, Step2, Step5, Step6.
...
export type Step1Answers = {
  ...
  capitalAmount?: number;       // used when lookingFor === capital_and_equipment
  equipmentAmount?: number;     // used when lookingFor === capital_and_equipment OR equipment
  fundingAmount?: number;       // used when lookingFor === capital
...
const revenue12Rule: Record<Revenue12Key, readonly Cat[]> = {
  ...
  "150-500k": ALL_CATEGORIES,
  "500k-1m":  ALL_CATEGORIES,
  "1m-3m":    ALL_CATEGORIES,
  ">3m":      ALL_CATEGORIES,
};
...
const avgMonthlyRule: Record<AvgMonthlyKey, readonly Cat[] | "BLOCK"> = {
  ...
  "25-50k":   ALL_CATEGORIES,
  "50-100k":  ALL_CATEGORIES,
  "100-250k": ALL_CATEGORIES,
  ">250k":    ALL_CATEGORIES,
};
...
export function detectHardStop(a: Step1Answers): HardStop | null {
  if (a.location === "OTHER") {
    return {
      ...
      message:
        "At this time we only fund corporations registered in either Canada or the United States.",
    };
  }
  if (a.avgMonthly === "<10k") {
    return {
      reason: "MIN_REVENUE",
      message:
        "Your business does not currently meet our minimum monthly revenue threshold. Please reach out once revenue has grown.",
    };
  }
  return null;
}
...
/** Categories allowed by every answered Step-1 question. Hard-stops short-circuit to []. */
export function computeAllowedCategories(a: Step1Answers): Cat[] {
  ...
  const apply = (rule: readonly Cat[] | undefined) => {
    if (rule) allowed = intersect(allowed, rule);
  };
...
/** True iff at least one active STARTUP product exists for the user's country. */
export function isStartupAvailable(
  products: NormalizedLenderProduct[],
  country: "CA" | "US" | "" | null | undefined
): boolean {
...
/** Closing-costs companion: 20% of equipment amount, TERM ≤ $50k else LOC. */
export function computeCompanion(equipmentAmount: number): {
  amount: number;
  category: "TERM" | "LOC";
} {
...
/** True iff a closing-costs companion product would actually match the equipment amount. */
export function canOfferClosingCosts(
  products: NormalizedLenderProduct[],
  ...
  return products.some((p) => {
    const cat = String((p as any).category ?? "").toUpperCase();
    if (cat !== category && cat !== (category === "TERM" ? "TERM_LOAN" : "LINE_OF_CREDIT")) {
      return false;
    }
    const c = String((p as any).country ?? "").toUpperCase();
    if (country && c !== country && c !== "BOTH" && c !== "") return false;
    const min = Number((p as any).amount_min ?? (p as any).amountMin ?? 0);
    const max = Number((p as any).amount_max ?? (p as any).amountMax ?? Infinity);
...
/** All "legs" the user is applying for. Each leg → its own application. */
export type Leg = {
  category: Cat;
  amount: number;
  isCompanion?: boolean;       // true when this leg is a closing-costs companion
  parentCategory?: Cat;        // EQUIPMENT, when isCompanion === true
};
...
export function buildLegs(input: {
  ...
  if (input.lookingFor === "capital_and_equipment") {
    if (input.selectedCapitalCategory) {
      legs.push({ category: input.selectedCapitalCategory, amount: input.capitalAmount });
    }
    legs.push({ category: "EQUIPMENT", amount: input.equipmentAmount });
    return legs;
  }
