...
import { getApplicationToken, clearApplicationToken } from "@/auth/applicationToken";
...
function readBootToken(): string | null { if(typeof window==="undefined") return null; try{const raw=getApplicationToken(); return raw&&typeof raw==="string"?raw:null;}catch{return null;} }
...
export function applyBrokerPrefill(token: string, part: Partial<ApplicationData>): void { _set(()=>{ const n=hydrateApplication({...emptyApp,...part,applicationToken:token} as ApplicationData); return {...n,applicationDraft:buildApplicationDraft(n)} as ApplicationData; }); }
...
export function useApplicationStore(){ const app=useSyncExternalStore(_subscribe,_getSnapshot,_getSnapshot); const update=useCallback((part:Partial<ApplicationData>)=>_set(prev=>{const n={...prev,...part} as ApplicationData; return {...n,applicationDraft:buildApplicationDraft(n)};}),[]); const reset=useCallback(()=>{_set(()=>emptyApp); OfflineStore.clear(); try{localStorage.removeItem(APPLICATION_STATE_KEY);localStorage.removeItem(CLIENT_BACKUP_KEY);}catch{} clearDraft(); clearSubmissionIdempotencyKey(); _trackedStep=undefined;},[]); const startNewApplication=useCallback(()=>{/* BF_CLIENT_BLOCK_v765_NEW_APP_PREFILL — a new application keeps Step 3 (business) + Step 4 (applicant/owners) and starts fresh on Step 1 (kyc), Step 2 (product/amount) and documents. Drop the boot token + draft + idempotency so the wizard mints a NEW application instead of resuming the prior one; the seeded profile is persisted by _set so a refresh keeps it. */ _set((prev)=>{/* BF_CLIENT_QA_FLOW_FIXES_v1 - keep identity, drop country-specific fields so a
   new app in another country does not inherit the prior address/phone/postal/
   structure/ID (e.g. a US SBA app inheriting a Canadian address). */
const OMIT_BIZ=["state","province","zip","postalCode","businessStructure","phone","businessPhone","address","street","city","businessName","mailingZip","mailingState","mailingProvince","mailingAddress"];
const OMIT_APP=["state","province","zip","postalCode","phone","homePhone","workPhone","street","city","ssn","sin","partnerState","partnerProvince","partnerZip"];
const stripKeys=(o:any,keys:string[])=>{const c={...(o||{})};for(const k of keys)delete (c as any)[k];return c;};
const seeded={...emptyApp,business:stripKeys(prev.business,OMIT_BIZ),applicant:stripKeys(prev.applicant,OMIT_APP)} as ApplicationData; return {...seeded,applicationDraft:buildApplicationDraft(seeded)};}); try{clearApplicationToken();}catch{} clearDraft(); clearSubmissionIdempotencyKey(); _trackedStep=undefined;},[]); const loadFromServer=useCallback((state:Partial<ApplicationData>)=>_set(prev=>hydrateApplication({...prev,...state} as ApplicationData)),[]); const setToken=useCallback((token:string)=>_set(prev=>{const n={...prev,applicationToken:token} as ApplicationData; return {...n,applicationDraft:buildApplicationDraft(n)};}),[]);
