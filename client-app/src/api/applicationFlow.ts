...
export async function continueReadiness(payload: unknown) {
  return apiRequest(API_ENDPOINTS_CONTRACT.READINESS.CONTINUE, {
    method: "POST",
    body: JSON.stringify(payload)
  });
}
...
export async function getReadinessSession(sessionId: string) {
  return apiRequest(`${API_ENDPOINTS_CONTRACT.READINESS.SESSION_PREFIX}${encodeURIComponent(sessionId)}`);
}
